-- Initial schema migrated from remote Supabase project
-- Generated via `supabase db dump` and copied here so `supabase db reset`
-- can recreate the schema locally for development and testing.
...
-- Ensure a profile row is created whenever a new auth user is inserted.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
